...
<script type="text/javascript">
	window.onload = function() {
		...
		rowcopy.onclick = function() {
			...
			var tbody = document.querySelector('#table > #member > tbody');
			var chk = document.getElementsByName('checkbox');
			request.open("GET", "membermanager", true);
			request.send(null);
			for (var i = 0; i < chk.length; i++)
				if (chk[i].checked) {
					var code = i;
					request.open("POST", "membersmanager", true);
		...
		var numClick = function(event) {
			...
			//page++;
			request.open("GET", "membersPartial?p=" + page + "&f=" + param.f
					+ "&q=" + param.q, true);
			request.send(null);
...
<body>
	...
	<p id="cur-page">
		...
		<span class="strong">${param.p}</span> / ${lastNum} page
	</p>
	<footer id="footer"> <my:pager /> </footer>
